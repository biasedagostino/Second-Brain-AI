...
export default function App() {
  ...
  useEffect(() => {
    document.title = "Second Brain AI";
  }, []);
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        setApiKeys(keys);
        localStorage.setItem('openrouter_keys', JSON.stringify(keys));
      ...
      {!user ? (
        ...
        <>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="md-card p-6 flex flex-col justify-between">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-md-on-surface-variant text-[10px] font-bold uppercase tracking-widest">Totale Note</span>
                    ...
                    <div className="md-card p-6 flex flex-col justify-between">
                            ...
                            note.connections?.forEach((conn: string) => {
                              notes.forEach(otherNote => {
                                if (note.id !== otherNote.id && otherNote.keywords.includes(conn.toLowerCase())) {
                                  links.push([note.id, otherNote.id].sort().join('-'));
                                }
                              });
                            });
                            // Shared keywords (overlap >= 2)
                            notes.forEach(otherNote => {
                      ...
                      <p className="text-[10px] text-md-on-surface-variant mt-2">Relazioni semantiche tra le note</p>
                    </div>

                    <div className="md-card p-6 flex flex-col justify-between">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-md-on-surface-variant text-[10px] font-bold uppercase tracking-widest">Keywords</span>
                        <div className="p-2 bg-green-500/10 rounded-lg text-green-500">
                          <Tag className="w-4 h-4" />
                        </div>
                      </div>
                      <div className="text-3xl font-bold">{new Set(notes.flatMap(n => n.keywords)).size}</div>
                      <p className="text-[10px] text-md-on-surface-variant mt-2">Tag unici estratti dall'AI</p>
                    </div>

                    <div className="md-card p-6 flex flex-col justify-between">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-md-on-surface-variant text-[10px] font-bold uppercase tracking-widest">Attività</span>
                        <div className="p-2 bg-orange-500/10 rounded-lg text-orange-500">
                          <RefreshCw className="w-4 h-4" />
                        </div>
                      </div>
                      <div className="text-xl font-bold truncate">
                        {notes.length > 0 ? format(notes[0].createdAt?.toDate?.() || new Date(), 'dd MMM', { locale: it }) : 'Nessuno'}
                      </div>
                      <p className="text-[10px] text-md-on-surface-variant mt-2">Ultima indicizzazione eseguita</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="md-card p-8 space-y-6">
                      <h3 className="text-xl font-bold">Distribuzione Contenuti</h3>
                      <div className="space-y-4">
                        {['note', 'website', 'document', 'image'].map(type => {
                    ...
                    {/* Top Keywords */}
                    <div className="md-card p-8 space-y-6">
                      <h3 className="text-xl font-bold">Tag più utilizzati</h3>
                      <div className="flex flex-wrap gap-2">
                        {Array.from(new Set(notes.flatMap(n => n.keywords)))
                          .map(tag => ({ tag, count: notes.filter(n => n.keywords.includes(tag)).length }))
                          ...
                          .map(({ tag, count }) => (
                            ...
                            >
                              <span className="text-xs font-bold text-md-primary uppercase tracking-tighter">#{tag}</span>
                              <span className="text-[10px] bg-md-primary text-white px-1.5 py-0.5 rounded-full">{count}</span>
                            </div>
                          ))}
                      </div>
                    </div>
                  ...
                  <div className="md-card p-8 space-y-6">
                    <div className="flex justify-between items-center">
                      <h3 className="text-xl font-bold">Attività Recente</h3>
                      <button onClick={() => setCurrentView('notes')} className="text-md-primary text-sm font-bold">Vedi tutte</button>
                    </div>
                    <div className="divide-y divide-md-outline/10">
                      {notes.slice(0, 5).map(note => (
                        <div key={note.id} className="py-4 flex items-center justify-between group cursor-pointer" onClick={() => setCurrentView('notes')}>
                          <div className="flex items-center gap-4">
                            <div className="p-2 bg-md-surface-variant/30 rounded-lg text-md-on-surface-variant">
                              {note.type === 'note' && <FileText className="w-4 h-4" />}
                              {note.type === 'website' && <LinkIcon className="w-4 h-4" />}
                              {note.type === 'image' && <ImageIcon className="w-4 h-4" />}
                            </div>
                            <div>
                              <p className="font-bold text-sm line-clamp-1">{note.title}</p>
                              <p className="text-[10px] text-md-on-surface-variant uppercase tracking-widest">
                                {note.createdAt?.toDate ? format(note.createdAt.toDate(), 'dd MMMM yyyy HH:mm', { locale: it }) : 'Ora'}
                              </p>
                            </div>
                          </div>
                          <ChevronRight className="w-5 h-5 text-md-outline opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0" />
                        </div>
                      ))}
                      {notes.length === 0 && <p className="text-sm text-md-on-surface-variant py-8 text-center">Nessuna attività registrata.</p>}
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {currentView === 'graph' && (
                <motion.div
                  ...
                  className="p-4 md:p-8 h-screen flex flex-col space-y-4"
                >
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 h-20 bg-md-surface/80 backdrop-blur-xl border-t border-md-outline/10 flex items-center justify-around px-6 pb-2 z-50">
            ...
            <button onClick={() => setCurrentView('notes')} className={cn("p-3 rounded-full transition-all flex flex-col items-center", currentView === 'notes' ? "bg-md-primary-container text-md-on-primary-container" : "text-md-on-surface-variant hover:bg-md-on-surface/5")}>
        ...
        {dialog && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/70 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="bg-md-surface-variant max-w-sm w-full p-8 rounded-[32px] border border-md-outline/20 shadow-2xl text-center"
            >
              <div className="w-16 h-16 bg-md-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
                <Brain className="w-8 h-8 text-md-primary" />
              </div>
              <h3 className="text-2xl font-bold mb-3">{dialog.title}</h3>
...
function NotesView({ onClose, user, existingNotes, setIsIndexing, isIndexing, setNotification, setProgress, setDialog, setCurrentView, initialShowForm = false }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'website' && content.startsWith('http')) {
        setNotification({ title: 'Scraping...', message: 'Estrazione contenuto dal sito web...', type: 'info' });
        try {
          ...
          if (scrapeResponse.ok) {
            ...
            if (scrapedText && scrapedText.length > 100) {
              ...
              throw new Error("Contenuto troppo breve o non estratto correttamente.");
            }
          } else {
            throw new Error(`Errore scraping: ${scrapeResponse.status}`);
          }
        } catch (scrapeError) {
      ...
      if (errorMessage.toLowerCase().includes('rate limit')) {
        setDialog({
          ...
          message: 'Hai superato il limite di richieste gratuite per questo modello su OpenRouter. Vuoi cambiare modello o API Key nelle impostazioni?',
        ...
        setNotification({
          ...
          message: 'Impossibile analizzare il contenuto. Riprova più tardi.',
          type: 'error'
        });
      }
    } finally {
    ...
    >
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        >
          ...
          {showForm ? 'Torna all\'elenco' : 'Nuova Nota'}
        </button>
      </header>
      ...
      {showForm ? (
        ...
        <div className="space-y-8">
          ...
          {filteredNotes.length === 0 && (
            <div className="h-64 flex flex-col items-center justify-center text-md-on-surface-variant opacity-40 space-y-4">
...
function QaView({ notes, user, isAsking, setIsAsking, setNotification, setProgress, setDialog, setCurrentView }: {
  ...
  const handleSaveAsNote = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'notes'), {
        title: `AI: ${result.q}`,
        content: result.a,
        summary: indexed.summary || result.a.slice(0, 100),
        type: 'note',
        keywords: indexed.keywords || ['ai-response'],
        connections: indexed.suggestedConnections || [],
        userId: user.uid,
        userEmail: user.email,
        createdAt: serverTimestamp(),
        updatedAt: serverTimestamp(),
      ...
      setNotification({
        title: 'Salvato!',
        message: 'La risposta è stata aggiunta alle tue fonti.',
        type: 'success'
      });
    } catch (error: any) {
      console.error(error);
      ...
      if (errorMessage.toLowerCase().includes('rate limit')) {
        setDialog({
          title: 'Limite Raggiunto',
          message: 'Hai superato il limite di richieste gratuite per questo modello. Vuoi cambiare modello o API Key nelle impostazioni?',
          confirmText: 'Vai alle Impostazioni',
          onConfirm: () => {
            setDialog(null);
            setCurrentView('settings');
        ...
        setNotification({
          title: 'Errore',
          message: 'Impossibile salvare la risposta.',
          type: 'error'
        });
      }
    } finally {
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-8 max-w-4xl mx-auto w-full">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-6 mb-24 md:mb-20 pb-12 pr-1 md:pr-4 sm-scrollbar">
        {!result && !isAsking && (
        ...
        {result && (
          ...
          >
            <div className="flex justify-end">
              <div className="bg-md-primary-container text-md-on-primary-container px-4 py-3 md:px-6 md:py-4 rounded-[20px] rounded-tr-none max-w-[90%] md:max-w-[80%] shadow-md">
                <p className="font-semibold text-xs md:text-base">{result.q}</p>
              </div>
            </div>
            <div className="flex flex-col items-start gap-3">
              <div className="bg-md-surface-variant/30 px-4 py-5 md:px-7 md:py-7 rounded-[24px] md:rounded-[32px] rounded-tl-none border border-md-outline/10 max-w-[100%] md:max-w-[90%] shadow-sm">
